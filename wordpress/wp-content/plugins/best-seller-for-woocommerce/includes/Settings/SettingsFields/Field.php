...
use GPLSCore\GPLS_PLUGIN_WOBTSLR\Settings\SettingsFields\{ TextAreaField, TextField, RepeaterField, SelectField, RadioField, CheckboxField };
	...
	public static function new_field( $id, $field, $apply_hooks = true ) {
		switch ( $field['type'] ) {
			...
			case 'email':
				...
				break;
			case 'url':
				return new UrlField( $id, $field, $apply_hooks );
				break;
			case 'color':
				return new ColorField( $id, $field, $apply_hooks );
				break;
			case 'text':
			case 'number':
			case 'datetime':
